...
/**
 * A class template representing the basic unit that can be scheduled for execution
...
class task
{
 ...
 public:
  ...
  task<typename impl::traits::parallel_result<result_t, TaskT...>::type
     , parameter_t>
   ...
   */
  template <typename... TaskT>
  task<typename impl::traits::sequence_result<result_t, TaskT...>::type
     , parameter_t>
...
class taskop
{
 ...
 public:
   ...
   * run via @ref task::run() method, the task will submit its @em Callable for
   * execution with the associated runner. The @em Callable must satisfy the
   * following requirements:
   *   - it must accept <tt>const std::weak_ptr<runner>&</tt> as its first
   *     parameter
   ...
   *     functor object as long as it provides only a single overload of the
   *     function call operator <tt>()</tt>.
   *
   * @note Unfortunately the functors returned by the @c std::bind template
  ...
  /**
   * Add am exception handler(s) to the task.
   */
...
template<typename ResultT, typename ParameterT>
template <typename... TaskT>
inline task<typename impl::traits::parallel_result<ResultT, TaskT...>::type
          , ParameterT>
...
template<typename ResultT, typename ParameterT>
template <typename... TaskT>
inline task<typename impl::traits::sequence_result<ResultT, TaskT...>::type
          , ParameterT>
