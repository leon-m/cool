...
// memory leak detection on windows
 ...
 # include <Windows.h>
#endif /* if defined ( _WIN32 ) && defined ( DEBUG ) && !defined ( __GNUC__ ) */
...
static inline void _assert_eval(
    const char *msg,
    char cond,
    const char *file,
    int line
