...
namespace ipv4 {
  ...
  const list_t assigned_list[] = {
    ...
    cool::net::ipv4::any,
    cool::net::ipv4::broadcast,
    cool::net::ipv4::rfc_broadcast,
    cool::net::ipv4::rfc_private_24,
    cool::net::ipv4::rfc_private_20,
    cool::net::ipv4::rfc_private_16,
    cool::net::ipv4::rfc_carrier_nat,
    cool::net::ipv4::rfc_loopback,
    cool::net::ipv4::rfc_unset,
    cool::net::ipv4::rfc_iana_private,
    cool::net::ipv4::rfc_test,
    cool::net::ipv4::rfc_test_2,
    cool::net::ipv4::rfc_test_3,
    cool::net::ipv4::rfc_6to4_anycast,
    cool::net::ipv4::rfc_test_comm,
    cool::net::ipv4::rfc_mcast,
...
namespace ipv6 {
  const list_t assigned_list[] = {
    ...
    cool::net::ipv6::rfc_ipv4map,
    cool::net::ipv6::rfc_teredo,
    cool::net::ipv6::rfc_ipv4translate,
    cool::net::ipv6::rfc_discard,
    cool::net::ipv6::rfc_doc,
    cool::net::ipv6::rfc_local,
...
cool::net::ipv4::binary_t _parse(std::istream& is)
{
  binary_t result;
  int count;
...
void parse(std::istream& is, cool::net::ipv4::host& val)
{
  val = static_cast<uint8_t*>(_parse(is));
}

void parse(std::istream& is, cool::net::ipv4::network& val)
{
  unsigned int mask = val.mask();
  auto addr = _parse(is);
  char c = is.get();
...
void parse(std::istream& is, ip::address& val)
{
  if (val.kind() == ip::HostAddress)
    parse(is, dynamic_cast<cool::net::ipv4::host&>(val));
  else
    parse(is, dynamic_cast<cool::net::ipv4::network&>(val));
}
...
cool::net::ipv6::binary_t _parse(std::istream& is)
{
  cool::net::ipv6::binary_t result, post_compress_value;
  ...
  result.set(result.size() - post_compress_index, post_compress_value);
  return result;
}

void parse(std::istream& is, cool::net::ipv6::host& val)
{
  val = static_cast<uint8_t*>(_parse(is));
}

void parse(std::istream& is, cool::net::ipv6::network& val)
{
  unsigned int mask = val.mask();
  auto addr = _parse(is);
  char c = is.get();
...
{
  if (val.kind() == ip::HostAddress)
    parse(is, dynamic_cast<cool::net::ipv6::host&>(val));
  else
...
bool host::equals(const ip::address &other) const
{
  ...
  if (kind() == ip::HostAddress)
  {
    if (in(rfc_ipv4map) || in(rfc_ipv4translate))
      return ::memcmp(&m_data[12], other, 4) == 0;
  }
  return false;
}
...
void host::visualize(std::ostream& os) const
{
  ...
  if (in(rfc_ipv4map) || in(rfc_ipv4translate))
    ...
    entrails::ipv6::str_canonical(m_data, ':', os, 16);
}
...
bool host::is(ip::Attribute a) const
{
  switch (a)
  {
    ...
    case ip::Assigned:
      return entrails::is_assigned(entrails::ipv6::assigned_list,
                                   entrails::ipv6::assigned_list_size,
                                   *this);
  }
  return false;
}
...
{
  if (::memcmp(&data, ipv6::rfc_ipv4map, 12) == 0 ||
      ::memcmp(&data, ipv6::rfc_ipv4translate, 12) == 0)
...
bool host::is(ip::Attribute a) const
{
  switch (a)
  {
    ...
    case ip::Ipv4Mapped:
      return false;
    case ip::Assigned:
      return entrails::is_assigned(entrails::ipv4::assigned_list,
                                   entrails::ipv4::assigned_list_size,
                                   *this);
  }
  return false;
}
    ...
    , m_length(mask_size)
{
  ...
  m_data = m_data & entrails::calculate_mask<4>(m_length);
}
...
bool network::equals(const ip::address &other) const
{
  if (kind() != other.kind() || version() != other.version())
    return false;
  return m_data == other && dynamic_cast<const network&>(other).m_length;
}
